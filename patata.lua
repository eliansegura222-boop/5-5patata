...
CategoryUI.Layout.Padding = UDim.new(0, MOBILE_DEVICE and 6 or 5)
...
local function createCategoryIcon(parent, categoryKey)
	...
	icon.Position = MOBILE_DEVICE and UDim2.new(0, 8, 0.5, 0) or UDim2.new(0, 12, 0.5, 0)
...
function CategoryUI:RefreshButtons()
	for key, button in pairs(self.Buttons) do
		...
		local accentBar = button:FindFirstChild("HexaCategoryAccentBar")
		local iconPlate = button:FindFirstChild("HexaCategoryIconPlate")
		local iconPlateStroke = iconPlate and iconPlate:FindFirstChild("HexaCategoryIconPlateStroke")
		...
		if MOBILE_DEVICE then
			...
			if accentBar then accentBar.Visible = false end
			if iconPlate then iconPlate.Visible = false end
			if icon then
			...
			-- PC: sidebar limpia, con placa de icono y selección mediante fondo/texto.
			button.BackgroundColor3 = selected and Color3.fromRGB(42, 42, 42) or Color3.fromRGB(26, 26, 26)
			button.BackgroundTransparency = selected and 0.01 or 0.22
			button.TextColor3 = selected and Theme.TextMain or Color3.fromRGB(220, 220, 220)
			if label and label:IsA("TextLabel") then
				label.TextColor3 = selected and Theme.TextMain or Color3.fromRGB(220, 220, 220)
			...
			if iconPlate then
				iconPlate.Visible = true
				iconPlate.BackgroundColor3 = selected and Color3.fromRGB(58, 58, 58) or Color3.fromRGB(34, 34, 34)
				iconPlate.BackgroundTransparency = selected and 0 or 0.06
			end
			if iconPlateStroke then
				iconPlateStroke.Color = selected and Theme.Accent or Color3.fromRGB(145, 145, 145)
				iconPlateStroke.Transparency = selected and 0.48 or 0.72
			end
			if icon then
				icon.Position = UDim2.new(0, 12, 0.5, 0)
				local iconColor = selected and Theme.TextMain or Color3.fromRGB(215, 215, 215)
				for _, object in ipairs(icon:GetDescendants()) do
					if object:IsA("Frame") then
						object.BackgroundColor3 = iconColor
					elseif object:IsA("UIStroke") then
						object.Color = iconColor
					end
				end
			end
			if stroke then
				stroke.Color = selected and Theme.Accent2 or Color3.fromRGB(115, 115, 115)
				stroke.Transparency = selected and 0.52 or 0.82
				stroke.Thickness = 1
			end
		end
	end
...
for index, definition in ipairs(CategoryUI.Definitions) do
	...
	button.Size = MOBILE_DEVICE and UDim2.fromOffset(math.max(92, #definition.Label * 6 + 46), 33) or UDim2.new(1, -8, 0, 34)
	button.BackgroundColor3 = MOBILE_DEVICE and Theme.Panel2 or Color3.fromRGB(26, 26, 26)
	button.BackgroundTransparency = MOBILE_DEVICE and 1 or 0.22
	...
	mkCorner(button, MOBILE_DEVICE and 15 or 9)
	local stroke = mkStroke(button, MOBILE_DEVICE and Theme.Purple or Color3.fromRGB(115, 115, 115), MOBILE_DEVICE and 0.62 or 0.82, 1)
	stroke.Name = "HexaCategoryStroke"

	-- Decoración exclusiva de PC: una placa sutil para el icono.
	-- La selección se indica mediante fondo/texto, sin barra vertical.
	local accentBar = Instance.new("Frame")
	accentBar.Name = "HexaCategoryAccentBar"
	...
	iconPlate.Name = "HexaCategoryIconPlate"
	iconPlate.AnchorPoint = Vector2.new(0, 0.5)
	iconPlate.Position = UDim2.new(0, 7, 0.5, 0)
	iconPlate.Size = UDim2.fromOffset(26, 26)
	iconPlate.BackgroundColor3 = Color3.fromRGB(34, 34, 34)
	iconPlate.BackgroundTransparency = 0.06
	iconPlate.BorderSizePixel = 0
	iconPlate.Visible = not MOBILE_DEVICE
	iconPlate.ZIndex = button.ZIndex + 1
	iconPlate.Parent = button
	mkCorner(iconPlate, 7)
	local iconPlateStroke = mkStroke(iconPlate, Color3.fromRGB(145, 145, 145), 0.72, 1)
	iconPlateStroke.Name = "HexaCategoryIconPlateStroke"

	createCategoryIcon(button, definition.Key)

	local label = Instance.new("TextLabel")
	label.Name = "HexaCategoryLabel"
	label.Position = MOBILE_DEVICE and UDim2.new(0, 30, 0, 0) or UDim2.new(0, 42, 0, 0)
	label.Size = MOBILE_DEVICE and UDim2.new(1, -36, 1, 0) or UDim2.new(1, -48, 1, 0)
	label.BackgroundTransparency = 1
	label.BorderSizePixel = 0
	label.Text = definition.Label
	label.TextColor3 = MOBILE_DEVICE and BUTTON_TEXT_COLOR or Color3.fromRGB(220, 220, 220)
	label.TextSize = MOBILE_DEVICE and 10 or 11
	label.Font = MOBILE_DEVICE and Enum.Font.GothamBold or Enum.Font.GothamMedium
	label.TextXAlignment = Enum.TextXAlignment.Left
	...
	-- El perfil se elige después de crear la GUI, así que el hover consulta
	-- MOBILE_DEVICE en tiempo real para no aplicar el estilo de PC en celular.
	local hoverTween = TweenInfo.new(0.14, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
	button.MouseEnter:Connect(function()
		if button:GetAttribute("HexaCategorySelected") == true then return end
		local targetColor = MOBILE_DEVICE and Theme.PurpleDeep or Color3.fromRGB(36, 36, 36)
		local targetTransparency = MOBILE_DEVICE and 0.14 or 0.06
		if PERFORMANCE_MODE then
			button.BackgroundColor3 = targetColor
			button.BackgroundTransparency = targetTransparency
		else
			tween(button, hoverTween, {BackgroundColor3 = targetColor, BackgroundTransparency = targetTransparency})
		end
	end)
	button.MouseLeave:Connect(function()
		local selected = button:GetAttribute("HexaCategorySelected") == true
		local targetColor
		local targetTransparency
		if MOBILE_DEVICE then
			targetColor = selected and Theme.PurpleDark or Theme.Panel2
			targetTransparency = selected and 0.04 or 1
		else
			targetColor = selected and Color3.fromRGB(42, 42, 42) or Color3.fromRGB(26, 26, 26)
			targetTransparency = selected and 0.01 or 0.22
		end
		if PERFORMANCE_MODE then
			button.BackgroundColor3 = targetColor
			button.BackgroundTransparency = targetTransparency
...
local function applyDeviceProfile(mode)
	...
	if MOBILE_DEVICE then
		...
		CategoryUI.Layout.Padding = UDim.new(0, 5)
	...
	for _, definition in ipairs(CategoryUI.Definitions) do
		...
		if button then
			...
			button.Size = MOBILE_DEVICE
				...
				or UDim2.new(1, -8, 0, 34)
			local corner = button:FindFirstChildOfClass("UICorner")
			if corner then corner.CornerRadius = UDim.new(0, MOBILE_DEVICE and 15 or 9) end
			local label = button:FindFirstChild("HexaCategoryLabel")
			if label and label:IsA("TextLabel") then
				label.Position = MOBILE_DEVICE and UDim2.new(0, 30, 0, 0) or UDim2.new(0, 42, 0, 0)
				label.Size = MOBILE_DEVICE and UDim2.new(1, -36, 1, 0) or UDim2.new(1, -48, 1, 0)
			...
			if accentBar then accentBar.Visible = false end
			local iconPlate = button:FindFirstChild("HexaCategoryIconPlate")
			if iconPlate then iconPlate.Visible = not MOBILE_DEVICE end
			local icon = button:FindFirstChild("HexaCategoryIcon")
			if icon then icon.Position = MOBILE_DEVICE and UDim2.new(0, 8, 0.5, 0) or UDim2.new(0, 12, 0.5, 0) end
		end
	end
...
CheckKeyBtn.MouseButton1Click:Connect(function()
	...
	if entered == "ikaH" then
		...
		t.Completed:Connect(continueAfterAccess)
	else
		KeyBox.Text = ""
