-- This script is almost entirely made by AI so you may run into issues, please report issues in the Discord.
-- Script and UI originally created by and uploaded by https://robloxscripts.com/user/Fleece
-- Join the Discord with this link: https://discord.gg/NyYzt9XVd9
...
-- Script and UI originally created by and uploaded by https://robloxscripts.com/user/Fleece
    ...
    if logoId then
        ...
        brand = text({
            Text = cfg.Title or "MONO", Font = FONT_BOLD, TextSize = 18, TextColor3 = T.text,
...
function MonoUI:SelectCategory(cat)
    if self.activeCategory == cat then
        if self._onVipCategory and cat.name == "VIP" then
            pcall(self._onVipCategory, self, cat, true)
        end
        return
    end
    ...
    tw(cat.iconImg, EASE, { ImageColor3 = (cat.name == "Favorites") and T.star or (cat.name == "VIP") and Color3.fromRGB(180, 220, 255) or T.text })
    self:Refresh(true)
    if self._onVipCategory then
        pcall(self._onVipCategory, self, cat, false)
    end
end
...
-- Script and UI originally created by and uploaded by https://robloxscripts.com/user/Fleece
    ...
    linkRow("GitHub", "github.com/fleecelolll", "hammer", "https://github.com/fleecelolll")
...
local Menu = MonoUI.new({
    ...
    LogoUrl = "https://raw.githubusercontent.com/fleecelolll/MonoMM2/main/Mono%20MM2%20Logo.png",
...
-- ========== UTILITY ==========
Utility:Section("Sheriff tools")
do
    local m = Utility:Module({ Name = "Auto Grab Gun", Desc = "Grabs the dropped gun the moment it appears, then returns you", Callback = function(v) flags.autoGun=v end, Info = "Grabs the dropped gun the moment it appears, then returns you." })
end

-- ========== EXTRA ==========
Extra:Section("Coins")
do
    local m = Extra:Module({ Name = "Auto Collect Coins", Desc = "Walks you coin to coin at normal speed", Callback = function(v) flags.autoCoins=v; if v then if coinBagFull then notify("Coin bag is already full") else notify("Auto collect on") end end end, Info = "Walks you coin to coin at normal speed. Idles while you are dead, in the lobby, or when no coins are out, and stops on its own when your bag is full." })
end
do
    local m = Extra:Module({ Name = "Teleport To Nearest Coin", Desc = "One hop to the closest uncollected coin", Action = true, Callback = function()
        local hrp=getHRP(LocalPlayer.Character)
...
do
    ...
    end, Info = "Blocks other exploiters from flinging you." })
end
do
    local m = Safety:Module({ Name = "Anti Trap", Desc = "Untested. Keeps your speed when you walk into a murderer trap", Callback = function(v) flags.antiTrap=v end, Info = "Untested. Murderer traps turn up rarely enough that this has never been confirmed working." })
end
Safety:Section("Awareness")
do
    local m = Safety:Module({ Name = "Murderer Notify", Desc = "Warns you when the murderer comes within 50 studs", Callback = function(v) flags.murdererNotify=v end, Info = "Pops a warning the moment the murderer gets within 50 studs of you." })
end

-- ========== VIP (Funciones + subtabs Configs / Players) ==========
VIP:Section("Funciones VIP")
do
    local m = VIP:Module({ Name = "Fling All Players", Desc = "Flings everyone in the server one after another, then puts you back", Action = true, Callback = flingAll, Info = "Flings everyone in the server one after another, then puts you back" })
end
do
    local m = VIP:Module({ Name = "Gun Through Walls", Desc = "Your sheriff shots ignore walls", Callback = function(v) flags.gunWalls=v end, Info = "Sheriff/hero shots ignore geometry so you can hit through cover." })
end
do
    local m = VIP:Module({ Name = "Aimbot", Desc = "Locks camera toward targets in FOV", Callback = function(v) flags.aimbot=v end, Info = "Smoothly aims your camera at targets inside the FOV circle." })
end
do
    local m = VIP:Module({ Name = "Noclip", Desc = "Walk through walls", Callback = function(v) flags.noclip=v end, Info = "Disables collision on your character so you can pass through geometry." })
end
do
    local m = VIP:Module({ Name = "Infinite Jump", Desc = "Jump again in mid-air", Callback = function(v) flags.infJump=v end, Info = "Lets you jump repeatedly while airborne." })
end
do
    local m = VIP:Module({ Name = "Fullbright", Desc = "Removes darkness and shadows", Callback = function(v) pcall(setFullbright, v) end, Info = "Forces bright lighting so dark maps are fully visible." })
end
do
    local m = VIP:Module({ Name = "FPS Boost", Desc = "Lowers effects for more FPS", Callback = function(v) pcall(setFPSBoost, v) end, Info = "Disables water waves and other heavy effects to raise frame rate." })
end

-- VIP sub-tabs: Funciones | Configs | Players (top of content when VIP is selected)
do
    local vipSub = "Funciones"
    local bar = Instance.new("Frame")
    bar.Name = "VIPSubTabs"
    bar.BackgroundTransparency = 1
    bar.Visible = false
    bar.Size = UDim2.new(1, -16, 0, 36)
    bar.Position = UDim2.new(0, 8, 0, 4)
    bar.ZIndex = 20
    -- parent near modList / content
    local contentParent = Menu.content or Menu.modList and Menu.modList.Parent
    if contentParent then
        bar.Parent = contentParent
    end
    ...
    local btnF = makeSub("Funciones")
    local btnC = makeSub("Configs")
    local btnP = makeSub("Players")

    local function paintSubs()
        for _, pair in ipairs({{btnF,"Funciones"},{btnC,"Configs"},{btnP,"Players"}}) do
            local b, n = pair[1], pair[2]
            local on = vipSub == n
            b.BackgroundTransparency = on and 0.1 or 0.45
            b.TextColor3 = on and Color3.fromRGB(255,255,255) or Color3.fromRGB(150,150,150)
            local st = b:FindFirstChildOfClass("UIStroke")
            if st then st.Color = on and Color3.fromRGB(180, 220, 255) or Color3.fromRGB(80,80,80) end
        end
    end

    local function applyVipSub()
        paintSubs()
        local isVip = Menu.activeCategory and Menu.activeCategory.name == "VIP"
        bar.Visible = isVip and true or false
        if not isVip then
            -- restore normal modules page when leaving VIP
            if Menu.pageConfigs then Menu.pageConfigs.Visible = false end
            if Menu.pagePlayers then Menu.pagePlayers.Visible = false end
            if Menu.content then Menu.content.Visible = true end
            if Menu.modList then Menu.modList.Visible = true end
            return
        end
        if vipSub == "Funciones" then
            if Menu.pageConfigs then Menu.pageConfigs.Visible = false end
            if Menu.pagePlayers then Menu.pagePlayers.Visible = false end
            if Menu.content then Menu.content.Visible = true end
            if Menu.modList then Menu.modList.Visible = true end
            Menu:Refresh(false)
        elseif vipSub == "Configs" then
            if Menu.content then Menu.content.Visible = false end
            if Menu.pagePlayers then Menu.pagePlayers.Visible = false end
            if Menu.pageConfigs then
                Menu.pageConfigs.Visible = true
                if Menu._refreshConfigs then pcall(Menu._refreshConfigs) end
            end
        elseif vipSub == "Players" then
            if Menu.content then Menu.content.Visible = false end
            if Menu.pageConfigs then Menu.pageConfigs.Visible = false end
            if Menu.pagePlayers then
                Menu.pagePlayers.Visible = true
                if Menu._rebuildPlayers then pcall(Menu._rebuildPlayers) end
            end
        end
    end

    btnF.MouseButton1Click:Connect(function() vipSub = "Funciones"; applyVipSub() end)
    btnC.MouseButton1Click:Connect(function() vipSub = "Configs"; applyVipSub() end)
    btnP.MouseButton1Click:Connect(function() vipSub = "Players"; applyVipSub() end)

    Menu._onVipCategory = function(ui, cat, same)
        if cat and cat.name == "VIP" then
            applyVipSub()
        else
            vipSub = "Funciones"
            applyVipSub()
...
-- Script and UI originally created by and uploaded by https://robloxscripts.com/user/Fleece
    ...
    Menu:AddTourStep({ Title = "Welcome to Mono",
...
-- Script and UI originally created by and uploaded by https://robloxscripts.com/user/Fleece
    ...
    if firstRun() then
        ...
        Mono.greet = notify("Mono loaded  " .. Mono.keyName() .. " to hide", 4)
        Mono.onMenuKey = function()
            local g = Mono.greet
            if g and g.SetContent then
                pcall(function() g:SetContent("Mono loaded  " .. Mono.keyName() .. " to hide") end)
            end
        end
    end
